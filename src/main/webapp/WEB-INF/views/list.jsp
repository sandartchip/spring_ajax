		...
		<ul>
			<c:if test="${ page_list_maker.prev }"> 
				<li style="float:left; list-style-type:none; padding-right:10px">
					<a href="<c:url value='list?pageNO=${page_list_maker.startPage-1}&search_text=${search_text}>'/>"> 이전 페이지 </a>
			...
			<c:forEach var="page_index" begin="${page_list_maker.startPage}" end="${page_list_maker.endPage}">
			 	<li style="float:left; list-style-type:none; padding-right:10px">
			 		<a href="<c:url value='list?pageNO=${page_index}&search_text=${search_text}'/>"> ${page_index}</a>  
			 	</li>
		 	</c:forEach>
		 	<c:if test="${ page_list_maker.next }">
		 		<li style="float:left; list-style-type:none;">
		 			<a href="<c:url value='list?pageNO=${page_list_maker.endPage+1}&search_text=${search_text}' />">다음 페이지</a>
		 		</li>
		 	</c:if>
	 	</ul>
	 	...
	 	<div class="search_box">
	 		...
	 		<form action="list">
		 		<input type="text" name="search_text"/> <!-- input box에 있는 데이터는 get방식 파라미터로 넘어간다. -->
